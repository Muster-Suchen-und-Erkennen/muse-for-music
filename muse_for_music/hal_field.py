...
class EmbeddedFields(Raw):
    ...
    def output(self, key, obj, orderes=False):
        ...
        for name in self.embedded_models:
            key = (
                name
                if not self.embedded_models[name].attribute
                else self.embedded_models[name].attribute
            )
            value = get_value(key, obj)
            if value is not None and not (
                self.embedded_models[name].as_list and (len(value) == 0)
            ):
...
class HaLUrl(StringMixin, Raw):
    ...
    def schema(self):
        ...
        props["href"] = {
            "type": "string",
            "readOnly": True,
            "example": "http://www.example.com/api",
        }

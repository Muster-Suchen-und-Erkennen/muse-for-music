    ...
    def has_roles_decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            for role in roles:
                ...
                auth_logger.debug(
                    "Access to ressource with isufficient rights. User roles: %s, required roles: %s",
...
def load_user_claims(user: User):
    return {"user_claims": [role for role in user.roles_json]}
...
from . import administration, authentication, root  # noqa
...
def register_user_api(app: Flask):
    ...
    fh = RotatingFileHandler(
        path.join(app.config["LOG_PATH"], "muse4music_auth.log"),
        maxBytes=104857600,
        backupCount=10,
    )

...
jwt_response = api.model("JWT", {"access_token": fields.String(required=True)})

jwt_response_full = api.inherit(
    "JWT_FULL", jwt_response, {"refresh_token": fields.String(reqired=True)}
)

user_model = api.model(
    "UserModel",
    {"username": fields.String(required=True), "roles": fields.List(fields.String())},
)
...
class Check(Resource):
    ...
    def get(self):
        ...
        roles = [r.name for r in RoleEnum if r.name in claims]
        ret = {"username": get_jwt_identity(), "roles": roles}
        return ret

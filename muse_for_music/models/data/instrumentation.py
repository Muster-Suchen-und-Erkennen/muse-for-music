...
class InstumentationToInstrument(db.Model):
    ...
    instrumentation = db.relationship(
        Instrumentation,
        backref=db.backref(
            "_instruments",
            lazy="selectin",
...
class InstrumentationContext(db.Model, GetByID, UpdateableModelMixin):
    ...
    id = db.Column(db.Integer, primary_key=True)
    instr_quantity_before_id = db.Column(
        db.Integer,
        db.ForeignKey("instrumentierung_einbettung_quantitaet.id"),
        nullable=True,
    )
    instr_quantity_after_id = db.Column(
        db.Integer,
        db.ForeignKey("instrumentierung_einbettung_quantitaet.id"),
        nullable=True,
    )
    instr_quality_before_id = db.Column(
        db.Integer,
        db.ForeignKey("instrumentierung_einbettung_qualitaet.id"),
        nullable=True,
    )
    instr_quality_after_id = db.Column(
        db.Integer,
        db.ForeignKey("instrumentierung_einbettung_qualitaet.id"),
        nullable=True,
    )

    instrumentation_quantity_before = db.relationship(
        InstrumentierungEinbettungQuantitaet, foreign_keys=[instr_quantity_before_id]
    )
    instrumentation_quantity_after = db.relationship(
        InstrumentierungEinbettungQuantitaet, foreign_keys=[instr_quantity_after_id]
    )
    instrumentation_quality_before = db.relationship(
        InstrumentierungEinbettungQualitaet, foreign_keys=[instr_quality_before_id]
    )
    instrumentation_quality_after = db.relationship(
        InstrumentierungEinbettungQualitaet, foreign_keys=[instr_quality_after_id]
    )

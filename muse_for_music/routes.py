...
from flask import Blueprint, Flask, Response, abort, redirect, render_template
...
def register_routes(app: Flask, flask_static_digest):  # noqa: C901
    ...
    def inject_csp_headers(response: Response):
        ...
        response.headers["Content-Security-Policy"] = (
            f"default-src 'self'; script-src {script_nonce} 'self'; style-src 'self' 'unsafe-inline'"
        )
        return response
    ...
    def index(path: str):
        ...
        return render_template(
            "index.html", title="muse4music", nonce=getattr(g, "nonce", "")
        )
    ...
    def asset(file):
        if "." in file:
            return redirect(
                flask_static_digest.static_url_for("static", filename="assets/" + file)
            )
        return render_template(
            "index.html", title="muse4music", nonce=getattr(g, "nonce", "")
        )

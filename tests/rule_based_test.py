...
from conftest import app, auth, client, taxonomies
...
from util import (
    ...
    compareObjects,
    get_hateoas_ref,
    get_hateoas_ref_from_object,
    get_hateoas_resource,
    try_self_link,
)
...
class ApiChecker(RuleBasedStateMachine):
    ...
    def add_referenced_by_reference(
        self, ref: ObjectReference, referenced_object: ObjectReference
    ):
        self.referenced_by_relations[ref].add(referenced_object)
        self.reference_counter[referenced_object] += 1

    def remove_referenced_by_reference(
        self, ref: ObjectReference, referenced_object: ObjectReference
    ):
    ...
    def remove_from_bundle(self, ref: ObjectReference):
        ...
        for i, name_ref in enumerate(bundle):
            ...
            if value["id"] == ref.id:
                ...
                check_deleted_result = self.client.get(
                    url, headers=auth_header(self.auth_token)
                )
                assert (
                    check_deleted_result.status_code == 404
                ), check_deleted_result.get_data().decode()
        assert (
            index_to_remove is not None
        ), 'The reference {} was not part of the bundle "{}" ({})!'.format(
            ref, bundle_name, bundle
        )
    ...
    def person_count_invariant(self):
        all_people = get_hateoas_resource(
            self.client, "person", auth=self.auth_token
        ).get_json()
    ...
    def opus_count_invariant(self):
        all_opera = get_hateoas_resource(
            self.client, "opus", auth=self.auth_token
        ).get_json()
    ...
    def part_count_invariant(self):
        all_parts = get_hateoas_resource(
            self.client, "part", auth=self.auth_token
        ).get_json()
    ...
    def can_post_person(self):
        if self.DEPTH_TO_CHECK > 0 and self.current_depth > 0:
            return False
        return (
            "POST" in self.known_rels.get("person", {})
            and self.is_authenticated()
            and len(self.objects_by_type[PERSON_TYPE]) < 10
        )
    ...
    def add_person(self, person):
        url = get_hateoas_ref(
            self.client, *self.known_rels["person"]["POST"], auth=self.auth_token
        )
    ...
    def update_person(self, old_person, person):
        ...
        if result.status_code == 409:
            assert (
                person["name"] in self.people_names
                and person["name"] != old_person["name"]
            )
            return old_person
        else:
            assert (
                person["name"] not in self.people_names
                or person["name"] == old_person["name"]
            )
            new_person = result.get_json()
            try_self_link(new_person, self.client, self.auth_token)
    ...
    def can_post_opus(self):
        ...
        return (
            can_post
            and bool(self.objects_by_type[PERSON_TYPE])
            and len(self.objects_by_type[OPUS_TYPE]) < 10
        )
    ...
    def add_opus(self, opus, composer):
        url = get_hateoas_ref(
            self.client, *self.known_rels["opus"]["POST"], auth=self.auth_token
        )
        ...
        if result.status_code == 409:
            ...
            self.add_reference(ref)
            self.add_referenced_by_reference(
                ref, ObjectReference(PERSON_TYPE, composer["id"])
            )
    ...
    def update_opus(self, old_opus, opus, composer):
        ...
        if result.status_code == 409:
            assert (
                opus["name"] in self.opus_names and opus["name"] != old_opus["name"]
            ), result.get_json()
            return old_opus
        else:
            ...
            if new_opus["composer"]["id"] != old_opus["composer"]["id"]:
                ...
                old_composer_ref = ObjectReference(
                    PERSON_TYPE, old_opus["composer"]["id"]
                )
    ...
    @rule(
        target=voices,
        old_voice=consumes(voices),
        voice=VOICE_PUT,
        cited_composers=st.lists(persons, unique_by=lambda p: p["id"]),
        data=st.data(),
    )
    def update_voice(self, old_voice=None, voice=None, cited_composers=[], data=None):
        ...
        old_composer_citations = set(
            [
                ObjectReference(PERSON_TYPE, p["id"])
                for p in old_voice["citations"]["composer_citations"]
            ]
        )
        new_composer_citations = set(
            [
                ObjectReference(PERSON_TYPE, p["id"])
                for p in voice["citations"]["composer_citations"]
            ]
        )
        ...
        old_opus_citations = set(
            [
                ObjectReference(OPUS_TYPE, c["opus"]["id"])
                for c in old_voice["citations"]["opus_citations"]
            ]
        )
        new_opus_citations = set(
            [
                ObjectReference(OPUS_TYPE, c["opus"]["id"])
                for c in voice["citations"]["opus_citations"]
            ]
        )
